...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if([profile.profileFireBuy isEqualToString:@"1"])
    {
...
-(void)setInAppThing
{
    ...
    [self.btnBuy setTitle:[NSString stringWithFormat:@"%@ (%@)",NSLocalizedString(@"sex_activate", nil),@"$2.99/yr"] forState:UIControlStateNormal];
    [self fetchInAppPurchases];
}

-(void)purchased:(NSNotification *)n
{
    if([n.object isEqualToString:INAPP_FIRE_ID])
    {
        [self removeLoaderView];
...
- (IBAction)actionBuy:(id)sender {
    [self setUpLoaderView:KOrangeColor];
    for (SKProduct *product in self.arrayInApp) {
        if([product.productIdentifier isEqualToString:INAPP_FIRE_ID])
        {
